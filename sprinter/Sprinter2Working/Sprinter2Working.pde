...
// G28 - Home all Axis                             <<<<------ be carefull if you have the directions inverted in the config !!!! Youll crash!
  ...
  if(buflen){
    ...
    if(savetosd){
        if(strstr(cmdbuffer[bufindr],"M29") == NULL){
           ...
           // Serial.println("ok"); //not the droid
        }else{
            file.sync();
            file.close();
            savetosd = false;
		...
		case 1:
              ...
              if(savetosd)
                break;
                  ...
                  //    Serial.println("ok"); //This is the droid!!!////////////////////////////////////////////////////////////
  ...
  else if(code_seen('M'))
  {
    ...
    switch( (int)code_value() ) 
    {
      ...
      case 106: //M106 Fan On
        if (code_seen('S')){
            WRITE(FAN_PIN, HIGH);
            analogWrite(FAN_PIN,90);// constrain(code_value(),0,255) );
        }
        else
            analogWrite(FAN_PIN,90); //WRITE(FAN_PIN, HIGH); Changed this because we have a 5V fan
        break;
...
void ClearToSend()
{
  ...
  if(fromsd[bufindr])
    return;
  #endif
  //Serial.println("ok"); // not the droids your are looking for
}
