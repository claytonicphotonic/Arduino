...
void loop() {
   ...
   if (step_size[0] >0){                     // must do this outside the loop because calculations are dependent on the size of the array
      for(i=0; i <= count; i++){
          num_steps = num_steps + pow(10,(count-1))*(step_size[i]-48);    //convert it to a base 10 number for us humans 
          step_size[i] = 0;                                                // clear out the memory for the next loop
          count--;                                                         
          
      }
      if(neg == true){
         num_steps = num_steps*-1;    // I don't know why but it is off by one step so I just added one here
         neg = false; 
      }
